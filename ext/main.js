...
async function main() {
	...
	const handler = e => {
		for (let rule of data.rules) {
			if (
				testMultipleRegexp(e.documentUrl, ...rule.origins) &&
				testMultipleRegexp(e.url, ...rule.domains)
			) {
				const frameHeader = find(
					e.responseHeaders,
					x => x.name === "x-frame-options"
				);
				const corsHeader = find(
					e.responseHeaders,
					x => x.name === "access-control-allow-origin"
				);
				if (frameHeader) {
